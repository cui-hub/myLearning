...
export default class Count extends Component {
  ...
  increment = ()=>{
    ...
    // 通知redux进行操作
    store.dispatch(createIncrementAction(Number(preValue)))
  }

  decrement = ()=>{
    const preValue = this.selectNumberElement.value;
    // 通知redux进行操作
    store.dispatch(createDecrementAction(Number(preValue)))
  }
  ...
  incrementAsync = ()=>{
    const preValue = this.selectNumberElement.value;
    // setTimeout(()=>{
    store.dispatch(createIncrementAsyncAction(Number(preValue), 1000))
    // },1000)
  }
